	...
	calculate = function(self, card, context)
		...
		if context.setting_blind then
			if not G["payasaka_lab_jokers_"..tostring(card.sort_id)] then
				G["payasaka_lab_jokers_"..tostring(card.sort_id)] = CardArea(0, 0, G.CARD_W * 2, G.CARD_H,
					{ card_limit = 2, type = 'joker', highlight_limit = 0 })
				---@type CardArea
				local dummy_area = G["payasaka_lab_jokers_"..tostring(card.sort_id)]
		...
		if context.end_of_round and context.cardarea == card.area and not context.game_over and not context.individual then
			...
			if G["payasaka_lab_jokers_"..tostring(card.sort_id)] then
				---@type CardArea
				local area = G["payasaka_lab_jokers_"..tostring(card.sort_id)]
